...
class BluetoothConnectionServer extends com.chaplin.realtimevoicetranslation.bluetooth.BluetoothConnection {
    //costants
    ...
    public BluetoothConnectionServer(final Context context, final String name, @NonNull final BluetoothAdapter bluetoothAdapter, final int strategy, final BluetoothConnectionClient client, final Callback callback) {
                        ...
                        if (newState == BluetoothProfile.STATE_CONNECTED) {
                            ...
                            synchronized (channelsLock) {
                                ...
                                if (!client.getConnectedPeers().contains(peer) && !channels.contains(peer)) {   // the client object is used to manage synchronization with the client to avoid adding a device that connects to the latter instead of us
                                    channels.add(new com.chaplin.realtimevoicetranslation.bluetooth.ServerChannel(context, peer, bluetoothAdapter));
                                    index = channels.size() - 1;
                                    ((com.chaplin.realtimevoicetranslation.bluetooth.ServerChannel) channels.get(index)).setBluetoothGattServer(bluetoothGattServer);
                            ...
                            synchronized (channelsLock) {
                                ...
                                if (index != -1) {
                                    ((com.chaplin.realtimevoicetranslation.bluetooth.ServerChannel) channels.get(index)).setBluetoothGattServer(null);
                        ...
                        synchronized (channelsLock) {
                            ...
                            if (characteristic.getUuid().equals(CONNECTION_REQUEST_UUID)) {
                                ...
                                if (index != -1) {
                                    if (!channels.get(index).getPeer().isDisconnecting()) {
                                        if (channels.get(index).getPeer().isReconnecting()) {
                                            if (!((com.chaplin.realtimevoicetranslation.bluetooth.ServerChannel) channels.get(index)).notifyConnectionResumed()) {
                                                stopReconnection(channels.get(index));
                                            }
                                            ...
                                            if (!((com.chaplin.realtimevoicetranslation.bluetooth.ServerChannel) channels.get(index)).notifyConnectionResumedRejected()) {
                                ...
                                if (index != -1) {
                                    if (!channels.get(index).getPeer().isDisconnecting()) {
                                        try {
                                            ...
                                            BluetoothGattService service = bluetoothGattServer.getService(com.chaplin.realtimevoicetranslation.bluetooth.BluetoothConnection.APP_UUID);
                                ...
                                if (index != -1) {
                                    ...
                                    com.chaplin.realtimevoicetranslation.bluetooth.BluetoothMessage subMessage = com.chaplin.realtimevoicetranslation.bluetooth.BluetoothMessage.createFromBytes(context, sender, value);
                                    if (subMessage != null) {
                                ...
                                if (index != -1) {
                                    Peer sender = (Peer) channels.get(index).getPeer().clone();
                                    com.chaplin.realtimevoicetranslation.bluetooth.BluetoothMessage subData = com.chaplin.realtimevoicetranslation.bluetooth.BluetoothMessage.createFromBytes(context, sender, value);
                                    if (subData != null) {
                ...
                mainHandler.post(new Runnable() {
                    ...
                    public void run() {
                        synchronized (channelsLock) {
                            ...
                            if (index != -1) {
                                UUID sendingCharacteristic = ((com.chaplin.realtimevoicetranslation.bluetooth.ServerChannel) channels.get(index)).getSendingCharacteristic();
        ...
        BluetoothGattService service = new BluetoothGattService(com.chaplin.realtimevoicetranslation.bluetooth.BluetoothConnection.APP_UUID, BluetoothGattService.SERVICE_TYPE_PRIMARY);
        ...
        mainHandler.post(new Runnable() {
            ...
            public void run() {
                synchronized (channelsLock) {
                    ...
                    if (index != -1) {
                        if (!channels.get(index).getPeer().isConnected() && !channels.get(index).getPeer().isReconnecting()) {
                            if (!((com.chaplin.realtimevoicetranslation.bluetooth.ServerChannel) channels.get(index)).acceptConnection()) {
                                channels.get(index).disconnect(disconnectionCallback);
                            }
                        }
                    }
            ...
            public void run() {
                synchronized (channelsLock) {
                    ...
                    if (index != -1) {
                        ...
                        if (!channels.get(index).getPeer().isConnected() && !channels.get(index).getPeer().isReconnecting()) {
                            channels.get(index).getPeer().setDisconnecting(true);
                            if (!((com.chaplin.realtimevoicetranslation.bluetooth.ServerChannel) channels.get(index)).rejectConnection()) {
                                channels.get(index).disconnect(disconnectionCallback);
                            }
                        }
                    }
