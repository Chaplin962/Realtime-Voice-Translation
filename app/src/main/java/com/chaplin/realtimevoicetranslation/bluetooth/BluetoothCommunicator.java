...
public class BluetoothCommunicator {
    ...
    private ArrayDeque<com.chaplin.realtimevoicetranslation.bluetooth.Message> pendingMessages = new ArrayDeque<>();
    private ArrayDeque<com.chaplin.realtimevoicetranslation.bluetooth.Message> pendingData = new ArrayDeque<>();
    // objects
    private Context context;
    @Nullable
    private BluetoothAdapter bluetoothAdapter;
    @Nullable
    private com.chaplin.realtimevoicetranslation.bluetooth.BluetoothConnectionServer connectionServer;
    @Nullable
            ...
            public synchronized void onScanResult(int callbackType, ScanResult result) {
                ...
                switch (callbackType) {
                    case ScanSettings.CALLBACK_TYPE_ALL_MATCHES: {
                        ...
                        if (result.getScanRecord() != null && connectionClient != null) {
                            ...
                            if (uniqueName != null && uniqueName.length() > 0) {
                                com.chaplin.realtimevoicetranslation.bluetooth.Peer peerFound = new com.chaplin.realtimevoicetranslation.bluetooth.Peer(device1, uniqueName, false);
                    ...
                    case ScanSettings.CALLBACK_TYPE_MATCH_LOST: {
                        BluetoothDevice device1 = result.getDevice();
                        notifyPeerLost(new com.chaplin.realtimevoicetranslation.bluetooth.Peer(device1, null, false));
                        break;
                    }
                }
            }
    ...
    private void initializeConnection() {
        if (bluetoothAdapter != null) {
            com.chaplin.realtimevoicetranslation.bluetooth.BluetoothConnection.Callback connectionCallback = new com.chaplin.realtimevoicetranslation.bluetooth.BluetoothConnection.Callback() {
                @Override
                public void onConnectionRequest(com.chaplin.realtimevoicetranslation.bluetooth.Peer peer) {
                    super.onConnectionRequest(peer);
                    notifyConnectionRequest(peer);
                }

                @Override
                public void onConnectionSuccess(com.chaplin.realtimevoicetranslation.bluetooth.Peer peer, int source) {
                    super.onConnectionSuccess(peer, source);
                    notifyConnectionSuccess(peer, source);
                }

                @Override
                public void onConnectionLost(com.chaplin.realtimevoicetranslation.bluetooth.Peer peer) {
                    super.onConnectionLost(peer);
                ...
                @Override
                public void onPeerUpdated(com.chaplin.realtimevoicetranslation.bluetooth.Peer peer, com.chaplin.realtimevoicetranslation.bluetooth.Peer newPeer) {
                    super.onPeerUpdated(peer, newPeer);
                    notifyPeerUpdated(peer, newPeer);
                }

                @Override
                public void onConnectionResumed(com.chaplin.realtimevoicetranslation.bluetooth.Peer peer) {
                    super.onConnectionResumed(peer);
                    synchronized (BluetoothCommunicator.this) {
                        if (connectionServer != null && connectionClient != null) {
                            if (connectionServer.getReconnectingPeers().size() == 0 && !advertising) {
                ...
                @Override
                public void onConnectionFailed(com.chaplin.realtimevoicetranslation.bluetooth.Peer peer, int errorCode) {
                    super.onConnectionFailed(peer, errorCode);
                    notifyConnectionFailed(peer, errorCode);
                }

                @Override
                public void onMessageReceived(com.chaplin.realtimevoicetranslation.bluetooth.Message message, int source) {
                    super.onMessageReceived(message, source);
                ...
                @Override
                public void onDataReceived(com.chaplin.realtimevoicetranslation.bluetooth.Message data, int source) {
                ...
                @Override
                public void onDisconnected(com.chaplin.realtimevoicetranslation.bluetooth.Peer peer) {
            ...
            connectionServer = new com.chaplin.realtimevoicetranslation.bluetooth.BluetoothConnectionServer(context, uniqueName, bluetoothAdapter, strategy, connectionClient, connectionCallback);
        }
    }
    ...
    private int executeStartAdvertising() {
        ...
        if (advertisementSupportedCode == SUCCESS) {
            ...
            AdvertiseData advertiseData = new AdvertiseData.Builder()
                    .addServiceUuid(new ParcelUuid(com.chaplin.realtimevoicetranslation.bluetooth.BluetoothConnection.APP_UUID))
    ...
    private int executeStartDiscovery() {
        if (bluetoothAdapter != null) {
            ...
            scanFilters.add(new ScanFilter.Builder()
                    .setServiceUuid(new ParcelUuid(com.chaplin.realtimevoicetranslation.bluetooth.BluetoothConnection.APP_UUID))
                    .build());
     ...
     */
    public void sendMessage(final com.chaplin.realtimevoicetranslation.bluetooth.Message message) {
        mainHandler.post(new Runnable() {
            @Override
            public void run() {
    ...
    private void sendMessage() {
        if (connectionClient != null && connectionServer != null) {
            final com.chaplin.realtimevoicetranslation.bluetooth.Message message = pendingMessages.peekFirst();
            if (message != null) {
                connectionClient.sendMessage(message, new com.chaplin.realtimevoicetranslation.bluetooth.Channel.MessageCallback() {
                    @Override
                    public void onMessageSent() {
                        connectionServer.sendMessage(message, new com.chaplin.realtimevoicetranslation.bluetooth.Channel.MessageCallback() {
                            @Override
     ...
     */
    public void sendData(final com.chaplin.realtimevoicetranslation.bluetooth.Message data) {
        mainHandler.post(new Runnable() {
            @Override
            public void run() {
    ...
    private void sendData() {
        if (connectionClient != null && connectionServer != null) {
            final com.chaplin.realtimevoicetranslation.bluetooth.Message data = pendingData.peekFirst();
            if (data != null) {
                connectionClient.sendData(data, new com.chaplin.realtimevoicetranslation.bluetooth.Channel.MessageCallback() {
                    @Override
                    public void onMessageSent() {
                        connectionServer.sendData(data, new com.chaplin.realtimevoicetranslation.bluetooth.Channel.MessageCallback() {
                            @Override
                            public void onMessageSent() {   // means that we have sent the message to all the client and server channels
     ...
     */
    public ArrayList<com.chaplin.realtimevoicetranslation.bluetooth.Peer> getConnectedPeersList() {
        ArrayList<com.chaplin.realtimevoicetranslation.bluetooth.Peer> connectedPeers = new ArrayList<>();
        if (connectionServer != null && connectionClient != null) {
     ...
     * @return SUCCESS if bluetooth le is supported by the device or BLUETOOTH_LE_NOT_SUPPORTED if not (or rarely if we had a generic bluetooth problem)
     */
    public int acceptConnection(com.chaplin.realtimevoicetranslation.bluetooth.Peer peer) {
        if (connectionServer != null) {
            connectionServer.acceptConnection((com.chaplin.realtimevoicetranslation.bluetooth.Peer) peer.clone());
            return SUCCESS;
        } else if(isBluetoothLeSupported() == BLUETOOTH_LE_NOT_SUPPORTED){
            return BLUETOOTH_LE_NOT_SUPPORTED;
        }else{
     ...
     * @return SUCCESS if bluetooth le is supported by the device or BLUETOOTH_LE_NOT_SUPPORTED if not (or rarely if we had a generic bluetooth problem)
     */
    public int rejectConnection(com.chaplin.realtimevoicetranslation.bluetooth.Peer peer) {
        if (connectionServer != null) {
            connectionServer.rejectConnection((com.chaplin.realtimevoicetranslation.bluetooth.Peer) peer.clone());
            return SUCCESS;
        } else if(isBluetoothLeSupported() == BLUETOOTH_LE_NOT_SUPPORTED){
            return BLUETOOTH_LE_NOT_SUPPORTED;
        }else{
     ...
     */
    public int connect(final com.chaplin.realtimevoicetranslation.bluetooth.Peer peer) {
        if (!destroying) {
            if (connectionClient != null) {
                connectionClient.connect((com.chaplin.realtimevoicetranslation.bluetooth.Peer) peer.clone());
                return SUCCESS;
            } else if(isBluetoothLeSupported() == BLUETOOTH_LE_NOT_SUPPORTED) {
                return BLUETOOTH_LE_NOT_SUPPORTED;
            } else {
    ...
    public int readPhy(com.chaplin.realtimevoicetranslation.bluetooth.Peer peer) {
        if (connectionClient != null && connectionServer != null) {
            connectionServer.readPhy((com.chaplin.realtimevoicetranslation.bluetooth.Peer) peer.clone());
            connectionClient.readPhy((com.chaplin.realtimevoicetranslation.bluetooth.Peer) peer.clone());
            return SUCCESS;
        } else if(isBluetoothLeSupported() == BLUETOOTH_LE_NOT_SUPPORTED){
            return BLUETOOTH_LE_NOT_SUPPORTED;
        }else{
     ...
     * @return SUCCESS if bluetooth le is supported by the device or BLUETOOTH_LE_NOT_SUPPORTED if not (or rarely if we had a generic bluetooth problem)
     */
    public int disconnect(final com.chaplin.realtimevoicetranslation.bluetooth.Peer peer) {
        synchronized (bluetoothLock) {
            if (connectionClient != null && connectionServer != null && bluetoothAdapter != null) {
                connectionServer.disconnect((com.chaplin.realtimevoicetranslation.bluetooth.Peer) peer.clone());
                connectionClient.disconnect((com.chaplin.realtimevoicetranslation.bluetooth.Peer) peer.clone());
    ...
    public int disconnectFromAll() {
        if (connectionClient != null && connectionServer != null) {
            connectionServer.disconnectAll(new com.chaplin.realtimevoicetranslation.bluetooth.Channel.DisconnectionNotificationCallback() {
                @Override
    ...
    private void notifyPeerFound(final com.chaplin.realtimevoicetranslation.bluetooth.Peer peer) {
        mainHandler.post(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < clientCallbacks.size(); i++) {
    ...
    private void notifyPeerLost(final com.chaplin.realtimevoicetranslation.bluetooth.Peer peer) {
        mainHandler.post(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < clientCallbacks.size(); i++) {
    ...
    private void notifyConnectionRequest(final com.chaplin.realtimevoicetranslation.bluetooth.Peer peer) {
        mainHandler.post(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < clientCallbacks.size(); i++) {
    ...
    private void notifyConnectionSuccess(final com.chaplin.realtimevoicetranslation.bluetooth.Peer peer, final int source) {
        mainHandler.post(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < clientCallbacks.size(); i++) {
    ...
    private void notifyConnectionFailed(final com.chaplin.realtimevoicetranslation.bluetooth.Peer peer, final int errorCode) {
        mainHandler.post(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < clientCallbacks.size(); i++) {
    ...
    private void notifyConnectionResumed(final com.chaplin.realtimevoicetranslation.bluetooth.Peer peer) {
        mainHandler.post(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < clientCallbacks.size(); i++) {
    ...
    private void notifyConnectionLost(final com.chaplin.realtimevoicetranslation.bluetooth.Peer peer) {
        mainHandler.post(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < clientCallbacks.size(); i++) {
    ...
    private void notifyMessageReceived(final com.chaplin.realtimevoicetranslation.bluetooth.Message message, final int source) {
        mainHandler.post(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < clientCallbacks.size(); i++) {
    ...
    private void notifyPeerUpdated(final com.chaplin.realtimevoicetranslation.bluetooth.Peer peer, final com.chaplin.realtimevoicetranslation.bluetooth.Peer newPeer) {
        mainHandler.post(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < clientCallbacks.size(); i++) {
    ...
    private void notifyDisconnection(final com.chaplin.realtimevoicetranslation.bluetooth.Peer peer, final int peersLeft) {
        mainHandler.post(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < clientCallbacks.size(); i++) {
    ...
    public static abstract class Callback extends com.chaplin.realtimevoicetranslation.bluetooth.BluetoothConnection.Callback {
        /**
         * Notify that advertise has started, if you want to do something after the start of advertising do it here, because
         * after startAdvertise there is no guarantee that advertise is really started (it is delayed)
         */
         ...
         */
        public void onPeerFound(com.chaplin.realtimevoicetranslation.bluetooth.Peer peer) {
        }
         ...
         * @param peer
         */
        public void onPeerLost(com.chaplin.realtimevoicetranslation.bluetooth.Peer peer) {
        }
